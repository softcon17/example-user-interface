  ...
  Polymer({
    ...
    _refresh_data(e, detail, sender) {
      ...
      if (e.srcElement.parentElement.id) {
        ...
        } else if (e.srcElement.parentElement.id === "UpdateMachineStatusRefreshBtn" || e.srcElement.parentElement.id === "MakeBookingRefreshBtn" || e.srcElement.parentElement.id === "MachineStatusRefreshBtn") {
    ...
    _launch_machine_post: function () {
      if (this.$['input-machine-name'].value && this.$['input-machine-location'].value) {
        ...
        this.displayAlert('Uh Oh!', 'There is no machine data to send!', 'important');
      }
    },
    ...
    _launch_booking_post: function () {
      if (this.$['drop-machine'].selected && this.$['input-job-id'].value && this.$['drop-timeslot'].selected && this.$['input-post-booking'].value) {
        ...
        this.displayAlert('Uh Oh!', 'There is no booking data to send!', 'important');
      }
    },
    ...
    _machineGetSuccess: function (event, ironRequest) {
      ...
      try {
        ...
        console.log(e);
        this.displayAlert('Uh Oh!', 'The machine data couldnt be parsed, See the console for details!', 'important');
      }
    },
    ...
    _bookingGetSuccess: function (event, ironRequest) {
      ...
      try {
        ...
        this.displayAlert('Uh Oh!', 'The booking data couldnt be parsed, See the console for details!', 'important');
      }
    },
    restSuccess: function (ironRequest) {
      this.displayAlert('Woo Hoo!', `${ironRequest.status} (${ironRequest.statusText})` + ` - Request to ${ironRequest.xhr.responseURL} completed successfuly!`, 'information');
    },
    _restFail: function (event, ironRequest) {

      if (ironRequest.request.__data__.status !== 500) {
        if (ironRequest.request.__data__.status === 0) {
          this.displayAlert('Uh Oh!', `404 - That endpoint doesn't exist`, 'important');
        } else {
          this.displayAlert('Uh Oh!', `${ironRequest.error} - ${ironRequest.request.__data__.statusText}`, 'important');
        }
      } else {
        this.displayAlert('Woo Hoo!', 'The endpoint exists!', 'information');
      }
    },
